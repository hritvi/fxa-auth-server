...
Lug.prototype.summary = function (request, response) {
  ...
  line.email = payload.email || query.email || (request.auth && request.auth.credentials && request.auth.credentials.email)
  line.phoneNumber = (response.source && response.source.formattedPhoneNumber)
